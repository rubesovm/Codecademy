...
const numberMenMedalists = country => {
  return `SELECT COUNT(DISTINCT name) as count FROM GoldMedal WHERE country = '${country}' AND gender = 'Men'`;
};
...
const mostMedaledAthlete = country => {
  return `SELECT name, COUNT(DISTINCT name) as count FROM GoldMedal WHERE country = '${country}' GROUP BY name ORDER BY count DESC LIMIT 1;`;
};
...
const orderedMedals = (country, field, sortAscending) => {
  let query = `SELECT * FROM GoldMedal WHERE country = '${country}'`;
  if (field) {
...
const orderedSports = (country, field, sortAscending) => {
  let query = `SELECT sport, COUNT(sport) as count, (COUNT(sport) * 100 /(SELECT COUNT(*) AS count FROM GoldMedal WHERE country = '${country}')) as percent FROM GoldMedal WHERE country = '${country}' GROUP BY sport`;
  if (field) {
    query += ` ORDER BY ${field}`
  }
